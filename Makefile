...
PERL = perl
PERL5_LIB = -L$(PERL5_INCLUDE) -lperl -ldl  -lpcre $(SYSLIBS)
...
ifeq (,$(PY3))
  PYTHON_INCLUDE= $(DEFS) -I/usr/include/python2.7 -I/usr/lib/python2.7/config
  PYTHON_LIB    = 
  PYTHON        = python $(PYTHON_FLAGS)
else
  PYTHON_INCLUDE= $(DEFS) -I/usr/include/python3.5m -I/usr/include/python3.5m
  PYTHON_LIB    = 
...
OCTAVE        = /usr/bin/octave-cli --no-window-system --silent --norc --no-history
OCTAVE_CXX    = $(DEFS)  -D_FORTIFY_SOURCE=2 -I/usr/include/octave-4.0.0/octave/.. -I/usr/include/octave-4.0.0/octave -I/usr/include/hdf5/serial -I/usr/include/octave-4.0.0/octave/.. -I/usr/include/octave-4.0.0/octave -I/usr/include/hdf5/serial -I/usr/include/mpi  -g -Wformat -Werror=format-security -O0

# Extra Octave specific dynamic linking options
OCTAVE_DLNK   =  -rdynamic -L/usr/lib/x86_64-linux-gnu/octave/4.0.0 -L/usr/lib/x86_64-linux-gnu  -loctinterp -loctave  -lutil -lm  
OCTAVE_SO     = .oct
...
GUILE_SO      = .so
GUILE_LIBS    = 
GUILE_LIBOPTS = -ldl  -lpcre $(SYSLIBS)
GUILE_LIBPREFIX = lib
...
JAVA_INCLUDE= -I"/usr/lib/jvm/java-1.5.0-gcj-5-amd64/include" -I"/usr/lib/jvm/java-1.5.0-gcj-5-amd64/include/linux"
...
OCC=$(COMPILETOOL) 
OCAMLDLGEN=$(COMPILETOOL) 
OCAMLFIND=$(COMPILETOOL) 
OCAMLMKTOP=$(COMPILETOOL)  $(SWIGWHERE)
NOLINK ?= false
...
ocaml_static: $(SRCDIR_SRCS)
	...
	$(NOLINK) || $(OCC) -g -ccopt -g -cclib -g -custom -o $(TARGET) swig.cmo $(INTERFACE:%.i=%.cmo) $(PROGFILE:%.ml=%.cmo) $(INTERFACE:%.i=%_wrap.o) $(OBJS) -cclib "$(LIBS)"

ocaml_dynamic: $(SRCDIR_SRCS)
	$(OCAMLCORE)
	$(SWIG) -ocaml $(SWIGOPT) -o $(ISRCS) $(INTERFACEPATH)
	$(OCC) -g -c -ccopt -g -ccopt "$(INCLUDES)" $(ISRCS) $(SRCDIR_SRCS)
	$(CXXSHARED) $(CPPFLAGS) $(CXXFLAGS) $(LDFLAGS) $(CCSHARED) -o $(INTERFACE:%.i=%.so) $(INTERFACE:%.i=%_wrap.o) $(OBJS) $(LIBS)
...
ocaml_static_toplevel: $(SRCDIR_SRCS)
	...
	$(OCC) -g -c $(INTERFACE:%.i=%.ml)
	test -z "$(PROGFILE)" || $(OCC) $(OCAMLPP) -c $(PROGFILE)
	$(NOLINK) || $(OCAMLMKTOP) swig.cmo -I $(OCAMLP4WHERE) camlp4o.cma swigp4.cmo -g -ccopt -g -cclib -g -custom -o $(TARGET)_top $(INTERFACE:%.i=%.cmo) $(INTERFACE:%.i=%_wrap.o) $(OBJS) -cclib "$(LIBS)"
...
ocaml_static_cpp: $(SRCDIR_SRCS)
	...
	$(OCC) -g -c $(INTERFACE:%.i=%.ml)
	test -z "$(PROGFILE)" || $(OCC) $(OCAMLPP) -c $(PROGFILE)
	$(NOLINK) || $(OCC) -g -ccopt -g -cclib -g -custom -o $(TARGET) swig.cmo $(INTERFACE:%.i=%.cmo) $(PROGFILE:%.ml=%.cmo) $(INTERFACE:%.i=%_wrap.o) $(OBJS) -cclib "$(LIBS)" -cc '$(CXX) -Wno-write-strings'
...
ocaml_static_cpp_toplevel: $(SRCDIR_SRCS)
	...
	$(OCC) -g -c $(INTERFACE:%.i=%.ml)
	test -z "$(PROGFILE)" || $(OCC) $(OCAMLPP) -c $(PROGFILE)
	$(NOLINK) || $(OCAMLMKTOP) swig.cmo -I $(OCAMLP4WHERE) dynlink.cma camlp4o.cma swigp4.cmo -g -ccopt -g -cclib -g -custom -o $(TARGET)_top $(INTERFACE:%.i=%.cmo) $(INTERFACE:%.i=%_wrap.o) $(OBJS) -cclib "$(LIBS)" -cc '$(CXX) -Wno-write-strings'
...
ocaml_dynamic_cpp: $(SRCDIR_SRCS)
	...
	$(CXXSHARED) $(CPPFLAGS) $(CXXFLAGS) $(LDFLAGS) -o $(INTERFACE:%.i=%.so) $(INTERFACE:%.i=%_wrap.o) $(OBJS) $(CPP_DLLIBS) $(LIBS)
	$(OCAMLDLGEN) $(INTERFACE:%.i=%.ml) $(INTERFACE:%.i=%.so) > $(INTERFACE:%.i=%_dynamic.ml)
	mv $(INTERFACE:%.i=%_dynamic.ml) $(INTERFACE:%.i=%.ml)
	rm $(INTERFACE:%.i=%.mli)
	$(OCAMLFIND) $(OCC) -g -c -package dl $(INTERFACE:%.i=%.ml)
...
RUBY_INCLUDE= 
RUBY_LIB     = /usr/include/ruby-2.3.0
RUBY_DLNK = 
RUBY_LIBOPTS = -lruby-2.3 -lpthread -lgmp -ldl -lcrypt -lm  -ldl  -lpcre $(SYSLIBS)
RUBY_SO = .so
RUBY = ruby
...
PIKE_LIB     = @PIKELIB@
PIKE_DLNK    = 
PIKE_LIBOPTS = @PIKELINK@ -ldl  -lpcre $(SYSLIBS)
